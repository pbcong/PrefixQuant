...
def train_one_epoch(qlayer, prefixed_key_values, attention_mask, position_ids,
                      ...
                      quant_inps, fp_inps_with_fp, fp_inps_with_quant, training_target, trainable_prefix=None):
    loss_list = []
    norm_list = []
    loss_components = {'task': [], 'smoothness': [], 'flatness': []}
    
    for index in range(len(quant_inps)):
        with traincast():
            input = quant_inps[index].to(dev)
            
            # Store query tensor if we're using trainable prefix
            q_input = None
            
            # Update prefixed_key_values if trainable prefix is enabled
            if trainable_prefix is not None:
                prefixed_key_values = trainable_prefix()
            ...
            # Capture query inputs for attention regularization if available
            if trainable_prefix is not None and hasattr(qlayer, 'self_attn') and hasattr(qlayer.self_attn, 'q_proj'):
                # Try to capture query tensor - this is model implementation dependent
                with torch.no_grad():
                    try:
                        # Typical implementation for transformer models
                        q_input = qlayer.self_attn.q_proj(input)
                        q_input = q_input.view(input.shape[0], -1, qlayer.self_attn.num_heads, qlayer.self_attn.head_dim)
                        q_input = q_input.transpose(1, 2)  # [batch, num_heads, seq, head_dim]
                    except:
                        # If can't capture, we'll use the alternative loss calculation
                        q_input = None
            
            # Calculate base task loss
            if training_target == 'fp_input':
                label = fp_inps_with_fp[index].to(dev)
                task_loss = loss_func(quant_out, label)
                ...
                task_loss = 1/2 * (loss_1 + loss_2)
                
            # If we're using trainable prefix, add regularization losses
            if trainable_prefix is not None and hasattr(trainable_prefix, 'get_smoothness_loss'):
                from utils.prefix_utils import compute_combined_loss
                # Use the combined loss with regularization
                loss, components = compute_combined_loss(
                    task_loss, 
                    trainable_prefix,
                    q_input=q_input,
                    weight_smoothness_alpha=getattr(optimizer, 'weight_smoothness_alpha', 0.01),
                    attention_flatness_alpha=getattr(optimizer, 'attention_flatness_alpha', 0.01)
                )
                
                # Store loss components for logging
                loss_components['task'].append(components['task_loss'])
                loss_components['smoothness'].append(components['smoothness_loss'])
                loss_components['flatness'].append(components['flatness_loss'])
            else:
                loss = task_loss
        ...
        params = trainable_parameters(qlayer)
        if trainable_prefix is not None:
            params = list(params) + list(trainable_prefix.parameters())
            
        norm = loss_scaler(loss, optimizer, parameters=params).cpu()
    ...
    # Create a log dict with regularization loss components if they exist
    log_dict = {}
    if loss_components['task']:
        log_dict['task_loss'] = sum(loss_components['task']) / len(loss_components['task'])
        log_dict['smoothness_loss'] = sum(loss_components['smoothness']) / len(loss_components['smoothness'])
        log_dict['flatness_loss'] = sum(loss_components['flatness']) / len(loss_components['flatness'])
    
    return loss_mean, norm_mean, log_dict
...
def eval_one_epoch(qlayer, prefixed_key_values, attention_mask, position_ids,
    ...
    for index in range(len(quant_inps)):
        with traincast():
            ...
            # If we're using trainable prefix, add regularization losses for evaluation
            if trainable_prefix is not None and hasattr(trainable_prefix, 'get_smoothness_loss'):
                from utils.prefix_utils import compute_combined_loss
                loss, components = compute_combined_loss(
                    task_loss,
                    trainable_prefix,
                    q_input=q_input,
                    weight_smoothness_alpha=0.01,  # Use default for evaluation
                    attention_flatness_alpha=0.01  # Use default for evaluation
                )
...
class CustomLRSchedule(object):
    def __init__(self, args, total_iter) -> None:
        ...
        if hasattr(args, 'prefix_lr') and args.prefix_lr > 0:
            empty_optimizer_3 = torch.optim.AdamW([torch.tensor(0)], lr=args.prefix_lr)
            self.prefix_scheduler = CosineAnnealingLR(empty_optimizer_3, T_max=total_iter, eta_min=args.prefix_lr/args.min_lr_factor)
            self.prefix_index = param_group_index
            param_group_index += 1
        else:
    ...
    def step(self, optimizer):
        ...
        if self.prefix_scheduler is not None:
            self.prefix_scheduler.step()
            optimizer.param_groups[self.prefix_index]['lr'] = self.prefix_scheduler.get_lr()[0]
...
def block_ap(
    ...
    # Get trainable prefix if available
    trainable_prefix = getattr(model, 'trainable_prefix_embedding', None)
    if trainable_prefix is not None:
        logger.info("Using trainable prefix embeddings during block adaptation")
        trainable_prefix = trainable_prefix.to(dev)
        
        # Log regularization parameters if using trainable prefix
        if hasattr(args, 'weight_smoothness_alpha'):
            logger.info(f"Weight smoothness regularization alpha: {args.weight_smoothness_alpha}")
        if hasattr(args, 'attention_flatness_alpha'):
            logger.info(f"Attention flatness regularization alpha: {args.attention_flatness_alpha}")
    ...
    with torch.no_grad():
        for i in range(iters):
            ...
            try:
                # If we have trainable prefix, update the prefixed_key_values
                if trainable_prefix is not None:
                    current_prefixed_key_values = trainable_prefix()
                else:
                    current_prefixed_key_values = prefixed_key_values
                    
                model(data.to(dev),past_key_values=get_kv_cache(current_prefixed_key_values, bs=args.batch_size))
            except ValueError:
                pass
    ...
    for block_index in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            # create optimizer
            assert args.quant_lr > 0 or args.weight_lr > 0 or (trainable_prefix is not None and args.prefix_lr > 0)
            ...
            if trainable_prefix is not None and args.prefix_lr > 0:
                param.append({"params":trainable_prefix.parameters(),"lr":args.prefix_lr})
                        
            lr_schedule = CustomLRSchedule(args, total_training_iteration)
            optimizer = torch.optim.AdamW(param, weight_decay=args.wd)
            
            # Store regularization weights for loss calculation
            if trainable_prefix is not None:
                optimizer.weight_smoothness_alpha = getattr(args, 'weight_smoothness_alpha', 0.01)
                optimizer.attention_flatness_alpha = getattr(args, 'attention_flatness_alpha', 0.01)

            loss_scaler = NativeScalerWithGradNormCount()
            trainable_number = trainable_parameters_num(qlayer)
            if trainable_prefix is not None:
                trainable_number += sum(p.numel() for p in trainable_prefix.parameters() if p.requires_grad)
            ...
            for epoch in range(args.epochs):
                ...
                train_loss, gradient_norm, log_dict = train_one_epoch(qlayer, prefixed_key_values, attention_mask, position_ids,
                      loss_scaler, loss_func, lr_schedule, optimizer, dev, traincast,
                      quant_train_inps, fp_train_inps_with_fp, fp_train_inps_with_quant, args.training_target,
                      trainable_prefix=trainable_prefix)
                val_loss, val_log_dict = eval_one_epoch(qlayer, prefixed_key_values, attention_mask, position_ids,
                      loss_func, dev, traincast,
                ...
                # Log regularization components if available
                if log_dict and 'task_loss' in log_dict:
                    logger.info(f"  Loss components - Task: {log_dict['task_loss']:.6f} | "
                                f"Weight Smoothness: {log_dict['smoothness_loss']:.6f} | "
                                f"Attention Flatness: {log_dict['flatness_loss']:.6f}")
                if val_log_dict and 'task_loss' in val_log_dict:
                    logger.info(f"  Val components - Task: {val_log_dict['task_loss']:.6f} | "
                                f"Weight Smoothness: {val_log_dict['smoothness_loss']:.6f} | "
                                f"Attention Flatness: {val_log_dict['flatness_loss']:.6f}")
        ...
        if args.epochs>0 or args.mse_init:
            ...
            if trainable_prefix is not None:
                # If we have trainable prefix, get the latest key-values
                current_prefixed_key_values = trainable_prefix()
            else:
                current_prefixed_key_values = prefixed_key_values
                
            update_dataset(qlayer,quant_train_inps, quant_train_inps,dev,attention_mask,position_ids,current_prefixed_key_values)
            update_dataset(qlayer,quant_val_inps, quant_val_inps,dev,attention_mask,position_ids,current_prefixed_key_values)
    ...
    # If we have trainable prefix, update the final prefixed_key_values for the model
    if trainable_prefix is not None:
        prefixed_key_values = trainable_prefix()
        model.trainable_prefix_embedding = trainable_prefix.cpu()  # Move back to CPU
    ...
    return model, prefixed_key_values
